...
export class Guard {
  ...
  public static greaterThanEqualTo(minValue: number, actualValue: number): Result<GuardResponse> {
    return actualValue >= minValue
      ? Result.ok<GuardResponse>()
      : Result.fail<GuardResponse>(`Number given {${actualValue}} is not greater than equal to {${minValue}}`);
  }

  public static lessThanEqualToDate(maxValue: Date, actualValue: Date): Result<GuardResponse> {
    return actualValue <= maxValue
      ? Result.ok<GuardResponse>()
      : Result.fail<GuardResponse>(`Date given {${actualValue}} is not less than {${maxValue}}`);
  }

  public static greateThanEqualToDate(maxValue: Date, actualValue: Date): Result<GuardResponse> {
    return actualValue >= maxValue
      ? Result.ok<GuardResponse>()
      : Result.fail<GuardResponse>(`Date given {${actualValue}} is not greater than {${maxValue}}`);
  }

  public static lessThanEqualTo(maxValue: number, actualValue: number): Result<GuardResponse> {
    return actualValue <= maxValue
      ? Result.ok<GuardResponse>()
      : Result.fail<GuardResponse>(`Number given {${actualValue}} is not less than equal to {${maxValue}}`);
  }

  public static lessThan(maxValue: number, actualValue: number): Result<GuardResponse> {
    return actualValue < maxValue
      ? Result.ok<GuardResponse>()
      : Result.fail<GuardResponse>(`Number given {${actualValue}} is not less than {${maxValue}}`);
  }
